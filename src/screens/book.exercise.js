...
function BookScreen({ user }) {
  ...
  const book = useBook(bookId, user.token)
  ...
  return (
    <div>
          ...
          <div css={{ display: 'flex', position: 'relative' }}>
            ...
            >
              {book.loadingBook ? null : (
                <StatusButtons token={user.id} bookId={book.id} />
              )}
            </div>
          </div>
      ...
      {!book.loadingBook && listItem ? (
        <NotesTextarea token={user.token} listItem={listItem} />
      ) : null}
    </div>
  )
}
...
function NotesTextarea({ listItem, token }) {
  const [mutate] = useUpdateListItem(token)
