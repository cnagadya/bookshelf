...
function StatusButtons({user, book}) {
  ...
  const updateCache = () => queryCache.invalidateQueries('list-items')
  ...
  const [update] = useMutation(
    async data => await client(`list-items/${data.id}`, { token: user.token, method: "PUT", data }), 
    {onSettled: updateCache}
  )
  // 🐨 call useMutation here and assign the mutate function to "remove"
  // the mutate function should call the list-items/:listItemId endpoint with a DELETE
  const [remove] = useMutation(async id => await client(`list-items/${id}`, { token: user.token, method: "DELETE" }),  
    {onSettled: updateCache}
  )
  ...
  const [create]= useMutation( async data => await client('list-items', { token: user.token, method: "POST", data }),
    {onSettled: updateCache}
  )
